...
export const RequestSettings: React.FC = () => {
    ...
    document.addEventListener('dialog-opened', function() {
        nodecg.sendMessage('ytSettings').then(result => {
    ...
    const handleDJNameChange = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
        setDJNameValue(event.target.value);
        DJName = event.target.value;
    }

    const handleMaxSongsChange = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
        setMaxSongsValue(event.target.value);
        maxSongs = event.target.value;
    }

    const handleMaxDurationChange = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
        setMaxDurationValue(event.target.value);
        maxDuration = event.target.value;
    }

    const handleVoteCountChange = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    ...
    return (
        <ThemeProvider theme={theme}>
            <TextField fullWidth label="DJ Name" variant="standard" value={DJNameValue} onChange={handleDJNameChange}/>
            <TextField fullWidth inputProps={{ inputMode: 'numeric', pattern: '[0-9]*' }} label="Maximum Songs" variant="standard" value={maxSongsValue} onChange={handleMaxSongsChange}/>
            <TextField fullWidth inputProps={{ inputMode: 'numeric', pattern: '[0-9]*' }} label="Maximum Duration" variant="standard" value={maxDurationValue} onChange={handleMaxDurationChange}/>
            <TextField fullWidth inputProps={{ inputMode: 'numeric', pattern: '[0-9]*' }} label="Vote Skip Count" variant="standard" value={voteCountValue} onChange={handleVoteCountChange}/>
        </ThemeProvider>
    )
}
